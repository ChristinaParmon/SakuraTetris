...
import com.example.tetris.helper.array2dOfBytes
...
class Appmodel {
    ...
    fun generateField(action: String){
        if (isGameActive()){
            ...
            if (!moveValid(coordinate as Point, frameNumber)){
                translateBlock (currentBlock?.position as Point, currentBlock?.frameNumber as Int)//фиксируем на месте
    ...
    private fun persistCellData(){ //состояния всех ячеек сохраняются
    ...
    private fun assessField (){ //построчное сканирование строк, проверка все ли заполнены
        for (i in 0 until field.size){
            var emptyCells = 0;
            for (j in 0 until field[i].size) {
    ...
    private fun shiftRows(nToRow: Int){//стираем
